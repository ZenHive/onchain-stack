...
- **Coverage floor is `--cover-threshold 0` and that is a scaffold placeholder, not a standard.** Raise it to a measured baseline as soon as real modules land. Per `critical-rules.md` § coverage tiers, `Analytics.*` and `Math.*` are critical-path (95% target) because they are pure and there is no excuse; read/binding layers target 80%.
