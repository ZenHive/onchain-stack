...
**Modules:** `Cartouche.Solana.PDA`, `Cartouche.Solana.ATA`, `Cartouche.Solana.TokenProgram`, `Cartouche.Solana.Token`
**Dependencies:** Phases 0-3, `:crypto` (SHA-256)
**Adds to:** `Cartouche.Solana.RPC` (one new method: `get_token_accounts_by_owner`)
...
```elixir
@token_program       Cartouche.Base58.decode!("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")
@token_2022_program  Cartouche.Base58.decode!("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb")
@ata_program         Cartouche.Base58.decode!("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL")
...
## Layer 4: `Cartouche.Solana.Token` (High-Level RPC + Utilities)

This is the "batteries included" module that combines PDA derivation, RPC calls, and instruction building into convenient functions. Analogous to `Cartouche.Erc20` on the Ethereum side.
...
def get_balance(wallet, mint, opts) do
  with {:ok, accounts} <- RPC.get_token_accounts_by_owner(wallet, [mint: mint], opts ++ [encoding: :json_parsed]) do
    case accounts do
      [] ->
        {:ok, %{amount: 0, decimals: 0, mint: Cartouche.Base58.encode(mint)}}
      accounts ->
        ...
        {:ok, %{amount: total, decimals: decimals, mint: Cartouche.Base58.encode(mint)}}
    end
  end
end
```
...
def transfer_instructions(from_wallet, to_wallet, mint, amount, decimals, opts) do
  {from_ata, _} = Cartouche.Solana.ATA.find_address(from_wallet, mint, opts)
  {to_ata, _} = Cartouche.Solana.ATA.find_address(to_wallet, mint, opts)

  # Create destination ATA if needed (idempotent - no-op if exists)
  create_ix = Cartouche.Solana.ATA.create_idempotent(from_wallet, to_wallet, mint, opts)

  # Transfer
  transfer_ix = Cartouche.Solana.TokenProgram.transfer_checked(
...
## Also in this Phase: `Cartouche.Solana.Programs` Constants
...
1. **`Cartouche.Solana.Programs`** - Central program ID constants (uses `~B58` sigil)

2. **`Cartouche.Solana.PDA`** - `find_program_address/2`, `create_program_address/2`
   - "On curve" check via Ed25519 point decompression (validated, uses `:crypto.mod_pow/3`)
   - Pure computation, no RPC

3. **`Cartouche.Solana.ATA`** - `find_address/3`, `create/4`, `create_idempotent/4`
   - Depends on PDA + Programs
   - Pure computation for find, instruction building for create

4. **`Cartouche.Solana.TokenProgram`** - instruction builders
   - `transfer/5`, `transfer_checked/7`, `approve/5`, `close_account/4`, `sync_native/2`
   - Pure instruction building, no RPC

5. **`Cartouche.Solana.RPC.get_token_accounts_by_owner/3`** - new RPC method

6. **`Cartouche.Solana.Token`** - high-level RPC + utilities
...
- **`Cartouche.Solana.Programs` for centralized constants**: Avoids scattered Base58 decoding and keeps program IDs in one place. Uses `~B58` sigil for compile-time decoding.
- **`Cartouche.Solana.Token` as the high-level module**: Combines RPC + computation, analogous to `Cartouche.Erc20`.
