...
`Cartouche.Application` currently only starts Ethereum signers from config. Add support for Solana signers:
...
Changes to `Cartouche.Application`:
- Read `:solana_signer` config alongside existing `:signer`
- Map `{:ed25519, key}` → `{Cartouche.Solana.Signer.Ed25519, :sign, [decoded_key]}`
- Map `{:cloud_kms, ...}` → `{Cartouche.Solana.Signer.CloudKMS, :sign, [...]}`
- Start `Cartouche.Solana.Signer` child specs with the mapped MFAs
- `:default` name maps to `Cartouche.Solana.Signer.Default`

## 2. SystemProgram: Use Programs Module

`Cartouche.Solana.SystemProgram` has its own `@system_program_id <<0::256>>` and `program_id/0`. Should delegate to `Cartouche.Solana.Programs.system_program/0` for consistency.
...
- Version bump to 1.6.0 (Solana support is additive, nothing breaks). Save 2.0.0 for the Ethereum namespace migration (`Cartouche.RPC` → `Cartouche.Ethereum.RPC`) which is an actual breaking change.
...
Quick audit of gaps:
- `Cartouche.Solana.RPC` - moduledoc exists, but individual method docs could use more examples
- `Cartouche.Solana.Token` - needs a concrete example in moduledoc showing a full balance query
- `Cartouche.Solana.Transaction` - moduledoc has a good example but could mention deserialization
...
- **`get_token_accounts_by_owner` RPC method**: Added in Phase 4 but only tested indirectly through `Cartouche.Solana.Token`. Add direct RPC test.
- **`Cartouche.Solana.RPC` option propagation**: Test that commitment, encoding, and other options are actually sent in the request params (requires a smarter mock that captures params).
