...
**Modules:** `Cartouche.Solana.Keys`, `Cartouche.Solana.Signer`, `Cartouche.Solana.Signer.Ed25519`, `Cartouche.Solana.Signer.CloudKMS`
**Dependencies:** OTP `:crypto` (built-in, OTP 24+), `Cartouche.Base58` (Phase 0), `google_api_cloud_kms` (optional, existing dep)
...
```elixir
defmodule Cartouche.Solana.Keys do
  @moduledoc """
...
### `Cartouche.Solana.Signer` (GenServer)

The Solana signer follows the same GenServer + MFA backend pattern as the Ethereum `Cartouche.Signer`, but is much simpler: no recovery bit brute-force, no chain ID encoding.
...
defmodule Cartouche.Solana.Signer do
  ...
  Delegates to a backend module via MFA (e.g., Cartouche.Solana.Signer.Ed25519
  for local keys, or Cartouche.Solana.Signer.CloudKMS for GCP KMS).
  ...
  def sign(message, name \\ Cartouche.Solana.Signer.Default)

  @doc "Get the 32-byte public key (Solana address) for this signer."
  @spec address(GenServer.name()) :: binary()
  def address(name \\ Cartouche.Solana.Signer.Default)
...
### `Cartouche.Solana.Signer.Ed25519` (local key backend)

Analogous to `Cartouche.Signer.Curvy` for Ethereum.
...
### `Cartouche.Solana.Signer.CloudKMS` (GCP KMS backend)

Analogous to `Cartouche.Signer.CloudKMS` for Ethereum. GCP KMS supports Ed25519 since April 2024 (algorithm: `EC_SIGN_ED25519`).
...
The existing `Cartouche.Signer.CloudKMS` (Ethereum/secp256k1) does three things differently:

| Aspect | Ethereum (`Cartouche.Signer.CloudKMS`) | Solana (`Cartouche.Solana.Signer.CloudKMS`) |
|---|---|---|
...
We create a new `Cartouche.Solana.Signer.CloudKMS` rather than trying to generalize the existing `Cartouche.Signer.CloudKMS`. Reasons:
...
```elixir
# Local key
config :cartouche, :solana_signer, [
...
`Cartouche.Application` would need a small addition to start Solana signers alongside Ethereum ones.

## Test Plan Additions (KMS)

### CloudKMS Tests

The existing `Cartouche.Signer.CloudKMS` tests (in `test/signer/cloud_kms_test.exs`) mock the GCP API. We'd follow the same pattern:
...
```
lib/cartouche/solana/keys.ex                # Cartouche.Solana.Keys
lib/cartouche/solana/signer.ex              # Cartouche.Solana.Signer (GenServer)
lib/cartouche/solana/signer/ed25519.ex      # Cartouche.Solana.Signer.Ed25519 (local key backend)
lib/cartouche/solana/signer/cloud_kms.ex    # Cartouche.Solana.Signer.CloudKMS (GCP KMS backend)
...
- **GenServer signer with MFA backends**: Same pattern as Ethereum's `Cartouche.Signer`, but simpler (no recovery bit, no chain ID). Caches public key, provides named process interface.
- **Separate CloudKMS module** (`Cartouche.Solana.Signer.CloudKMS`): Not a refactor of the Ethereum one. Every step differs (request field, hashing, PEM parsing, signature format). The modules are small enough that separate implementations are clearer than a shared abstraction.
...
- **Should `Cartouche.Solana.Keys` validate the seed-to-pubkey relationship in `from_keypair_bytes/1`?** Yes - we should derive the pubkey from the seed and compare. A mismatched keypair would cause silent signing failures.
