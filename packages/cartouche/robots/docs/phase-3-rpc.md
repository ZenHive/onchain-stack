...
**Module:** `Cartouche.Solana.RPC`
**Dependencies:** `Cartouche.Base58` (Phase 0), `Cartouche.Solana.Transaction` (Phase 2), Finch (existing)
**Also:** Extract shared JSON-RPC transport from existing `Cartouche.RPC`

---

## Overview

Solana uses JSON-RPC 2.0, the same transport as Ethereum. We can reuse the Finch HTTP client and JSON-RPC framing from the existing `Cartouche.RPC`, but all methods, parameters, and response types are Solana-specific.

### Key Differences from Ethereum RPC

| Aspect | Ethereum (`Cartouche.RPC`) | Solana (`Cartouche.Solana.RPC`) |
|---|---|---|
...
Before building `Cartouche.Solana.RPC`, extract the reusable parts from `Cartouche.RPC` into a shared module.

### `Cartouche.RPC.Transport` (or similar)

The following from `Cartouche.RPC` is generic and reusable:
...
What is NOT reusable (stays in `Cartouche.RPC`):
...
```elixir
defmodule Cartouche.RPC.Transport do
  @moduledoc """
...
**Decision**: This extraction should be a refactor of `Cartouche.RPC` that preserves its public API. The existing `Cartouche.RPC.send_rpc/3` can delegate to `Cartouche.RPC.Transport.send_rpc/4` with the Ethereum node URL.
...
```elixir
defmodule Cartouche.Solana.AccountInfo do
  @type t :: %__MODULE__{
...
defmodule Cartouche.Solana.SignatureStatus do
  @type t :: %__MODULE__{
...
defmodule Cartouche.Solana.TokenAmount do
  @type t :: %__MODULE__{
...
defmodule Cartouche.Solana.PrioritizationFee do
  @type t :: %__MODULE__{
    slot: non_neg_integer(),
...
@spec get_account_info(pubkey :: binary(), opts :: keyword()) ::
  {:ok, Cartouche.Solana.AccountInfo.t() | nil} | {:error, term()}
...
@spec get_signature_statuses(signatures :: [String.t()], opts :: keyword()) ::
  {:ok, [Cartouche.Solana.SignatureStatus.t() | nil]} | {:error, term()}
...
@spec get_token_account_balance(pubkey :: binary(), opts :: keyword()) ::
  {:ok, Cartouche.Solana.TokenAmount.t()} | {:error, term()}
...
@spec get_token_accounts_by_owner(owner :: binary(), filter :: keyword(), opts :: keyword()) ::
  {:ok, [%{pubkey: binary(), account: Cartouche.Solana.AccountInfo.t()}]} | {:error, term()}
...
#### `send_transaction/2`
```elixir
@spec send_transaction(transaction :: binary() | Cartouche.Solana.Transaction.t(), opts :: keyword()) ::
...
```elixir
@spec simulate_transaction(transaction :: binary() | Cartouche.Solana.Transaction.t(), opts :: keyword()) ::
  {:ok, map()} | {:error, term()}
...
@spec get_recent_prioritization_fees(addresses :: [binary()]) ::
  {:ok, [Cartouche.Solana.PrioritizationFee.t()]} | {:error, term()}
...
#### `send_and_confirm/2`
```elixir
@spec send_and_confirm(transaction :: Cartouche.Solana.Transaction.t(), opts :: keyword()) ::
  {:ok, String.t()} | {:error, term()}
...
Following existing Cartouche patterns (e.g., `Cartouche.Block.deserialize/1`, `Cartouche.Receipt.deserialize/1`), each response struct has a `deserialize/1` function that takes the raw JSON-decoded map.
...
test "get_balance returns lamports" do
  ...
  {:ok, balance} = Cartouche.Solana.RPC.get_balance(some_pubkey)
...
```
lib/cartouche/rpc/transport.ex              # Shared JSON-RPC transport
lib/cartouche/solana/rpc.ex                 # Cartouche.Solana.RPC
lib/cartouche/solana/account_info.ex        # AccountInfo struct
lib/cartouche/solana/signature_status.ex    # SignatureStatus struct
lib/cartouche/solana/token_amount.ex        # TokenAmount struct
lib/cartouche/solana/prioritization_fee.ex  # PrioritizationFee struct
...
- **Extract shared transport**: `Cartouche.RPC.Transport` handles JSON-RPC framing. Both `Cartouche.RPC` and `Cartouche.Solana.RPC` use it.
...
- **WebSocket subscriptions?** Solana supports `accountSubscribe`, `signatureSubscribe`, etc. These would be a separate module (`Cartouche.Solana.WS`). Out of scope for phase 3.
