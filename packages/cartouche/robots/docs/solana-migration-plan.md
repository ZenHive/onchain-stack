...
1. **`Cartouche.Base58`** lives at top level (not under `Cartouche.Solana`) - it's a generic encoding like `Cartouche.Hex`
...
4. **Separate `Cartouche.Solana.Signer.CloudKMS`** module - not shared with Ethereum's KMS signer. Every step differs (request field, PEM parsing, signature format). Small modules, clear separation.
5. **Extract `Cartouche.RPC.Transport`** - Shared JSON-RPC framing used by both Ethereum and Solana RPC
...
## Current State of Cartouche

Cartouche is a lightweight Ethereum RPC client for Elixir (v1.5.0). Everything is Ethereum-specific and lives directly under the `Cartouche.*` namespace with no chain-level namespacing.

### Module Inventory

| Module | What It Does | Ethereum-Specific? |
|---|---|---|
| `Cartouche.RPC` | JSON-RPC client (30+ methods), transaction sending, calling, gas estimation | Yes (methods, error decoding, gas logic) |
| `Cartouche.Transaction` | Build/encode/sign V1 (legacy) and V2 (EIP-1559) transactions | Yes (RLP, EIP-155, EIP-1559) |
| `Cartouche.Signer` | GenServer that wraps signing backends, finds recovery bit | Yes (secp256k1, recovery bit, EIP-155 v-value) |
| `Cartouche.Signer.Curvy` | Private key signing via Curvy (secp256k1) | Yes (secp256k1, keccak digest) |
| `Cartouche.Signer.CloudKMS` | Google Cloud KMS signing | Yes (secp256k1 key type) |
| `Cartouche.Recover` | Signature recovery and verification | Yes (secp256k1 recovery, keccak, EIP-191) |
| `Cartouche.Keys` | Keypair generation | Yes (secp256k1, keccak address derivation) |
| `Cartouche.Hex` | Hex encoding/decoding, `~h` sigil, EIP-55 checksummed addresses | Mostly (hex is generic, checksummed addresses are Ethereum) |
| `Cartouche.Hash` | Keccak-256 hashing | Yes (Keccak is Ethereum's hash) |
| `Cartouche.Util` | Wei conversion, chain IDs, address derivation, padding | Mostly Ethereum |
| `Cartouche.Typed` | EIP-712 typed data encoding | Yes |
| `Cartouche.Block` | Block deserialization | Yes |
| `Cartouche.Receipt` | Transaction receipt deserialization | Yes |
| `Cartouche.Trace` / `TraceCall` / `DebugTrace` | Trace deserialization | Yes |
| `Cartouche.FeeHistory` | EIP-1559 fee history | Yes |
| `Cartouche.Filter` | Event log filtering via `eth_newFilter` | Yes |
| `Cartouche.Erc20` | ERC-20 token wrapper | Yes |
| `Cartouche.Assembly` / `Cartouche.VM` | EVM assembler and pure VM | Yes |
| `Cartouche.OpenChain` | 4byte signature lookup | Yes |
| `Cartouche.Sleuth` | Contract call helper | Yes |
| `Cartouche.Application` | Supervision tree, config | Partially (starts signers, Finch) |
| `Mix.Cartouche.Gen` | ABI code generation | Yes |
...
1. **Signer GenServer pattern**: `Cartouche.Signer` is a GenServer that delegates to an `{mod, fun, args}` triple. The signing backend (Curvy, CloudKMS) returns a `%Curvy.Signature{}` struct, and the Signer wraps it with recovery bit logic and EIP-155 encoding. This is tightly coupled to Ethereum's secp256k1 + recovery bit model.
...
3. **Config-driven**: Node URL, chain ID, and signers are configured via `Application.get_env(:cartouche, ...)`.
...
Recommendation: Implement it ourselves. The algorithm is trivial (repeated divmod by 58) and avoids a dependency for something so small. We can put it in `Cartouche.Solana.Base58` or `Cartouche.Base58`.
...
lib/cartouche/
  ...
  rpc.ex                    # Cartouche.RPC (Ethereum)
  transaction.ex            # Cartouche.Transaction (Ethereum)
  signer.ex                 # Cartouche.Signer (Ethereum-specific GenServer)
  ...

  # New shared/generic modules
  base58.ex                 # Cartouche.Base58

  # New Solana modules
  solana/
    rpc.ex                  # Cartouche.Solana.RPC
    keys.ex                 # Cartouche.Solana.Keys (Ed25519 keypair generation)
    transaction.ex          # Cartouche.Solana.Transaction (build, serialize, sign)
    signer.ex               # Cartouche.Solana.Signer (GenServer, same MFA pattern as Ethereum)
    signer/
      ed25519.ex            # Cartouche.Solana.Signer.Ed25519 (local key backend)
      cloud_kms.ex          # Cartouche.Solana.Signer.CloudKMS (GCP KMS Ed25519 backend)
    system_program.ex       # Cartouche.Solana.SystemProgram (transfer, create account, etc.)
```
...
```
  ethereum/
    rpc.ex                  # Cartouche.Ethereum.RPC (moved from Cartouche.RPC)
    transaction.ex          # etc.
    ...
```
...
**1a. Base58 encoding/decoding** (`Cartouche.Base58`)
...
**1b. Ed25519 key management** (`Cartouche.Solana.Keys`)
...
**1c. Ed25519 signing** (`Cartouche.Solana.Signer`, `Cartouche.Solana.Signer.Ed25519`, `Cartouche.Solana.Signer.CloudKMS`)
...
**2a. Compact array encoding** (within `Cartouche.Solana.Transaction`)
- Encode/decode compact u16 (variable-length integer for array sizes)
- Used throughout transaction serialization

**2b. Transaction building and serialization** (`Cartouche.Solana.Transaction`)
...
**2c. System program helpers** (`Cartouche.Solana.SystemProgram`)
...
**3a. Core RPC** (`Cartouche.Solana.RPC`)
- Reuse Finch HTTP client from existing Cartouche infrastructure
- JSON-RPC 2.0 transport (same as Ethereum, can share `get_body/3` logic)
- Configurable node URL (e.g., `config :cartouche, :solana_node, "https://api.mainnet-beta.solana.com"`)
...
- Wire up Solana node + signer config in `Cartouche.Application`
- Documentation and README updates
- Consider the Ethereum namespacing migration (`Cartouche.RPC` → `Cartouche.Ethereum.RPC`)

### Phase 4: Tokens (see [phase-4-tokens.md](phase-4-tokens.md))

- PDAs (`Cartouche.Solana.PDA`) - `find_program_address`
- ATAs (`Cartouche.Solana.ATA`) - derive addresses, create instructions
- SPL Token Program instructions (`Cartouche.Solana.TokenProgram`)
- High-level token RPC (`Cartouche.Solana.Token`) - `get_balance`, `get_all_balances`, `transfer_instructions`
...
- **Anchor IDL parsing**: Code generation from IDL files (like `mix cartouche.gen` for ABI)
...
**Decision: Separate `Cartouche.Solana.Signer` GenServer, same MFA pattern, simpler internals.**

The Ethereum `Cartouche.Signer` GenServer is deeply Ethereum-specific (recovery bits, EIP-155, `%Curvy.Signature{}`). Can't be generalized without breaking it. Instead, `Cartouche.Solana.Signer` follows the same GenServer + MFA backend pattern but is much simpler:
...
Two backends:
- `Cartouche.Solana.Signer.Ed25519` - local key (analogous to `Cartouche.Signer.Curvy`)
- `Cartouche.Solana.Signer.CloudKMS` - GCP KMS with `EC_SIGN_ED25519` algorithm (analogous to `Cartouche.Signer.CloudKMS`, but uses `data` field instead of `digest`, raw 64-byte signatures instead of DER, and Ed25519 PEM parsing instead of EC point extraction)
...
**Options:**
- **(a)** Copy the HTTP/JSON-RPC plumbing into `Cartouche.Solana.RPC` (some duplication)
- **(b)** Extract `Cartouche.RPC.Base` or `Cartouche.JsonRPC` with shared transport, chain-specific modules call into it
...
Currently: `config :cartouche, :ethereum_node, "..."` and `config :cartouche, :chain_id, ...`

For Solana: Need `config :cartouche, :solana_node, "https://api.mainnet-beta.solana.com"` (or devnet/testnet variants).
...
`Cartouche.Base58` (top-level, since it's a generic encoding) vs `Cartouche.Solana.Base58` (namespaced under Solana).

Recommendation: `Cartouche.Base58` - it's a general-purpose encoding like `Cartouche.Hex`. Could potentially be useful for Bitcoin or other chains too.
...
The existing Ethereum code stays untouched. New Solana modules live under `Cartouche.Solana.*`. A shared JSON-RPC transport layer can be extracted from the existing `Cartouche.RPC` to avoid duplication.
