...
```
lib/cartouche/base58.ex           # Cartouche.Base58
test/base58_test.exs           # tests
```

## Decisions

- **Module location**: `Cartouche.Base58` (top-level, not under `Cartouche.Solana`) since it's a generic encoding like `Cartouche.Hex`.
- **No external dependency**: The algorithm is ~40 lines and we avoid a dep.
- **API style**: Match existing Cartouche patterns. `encode/1` returns a string, `decode/1` returns `{:ok, binary} | {:error, term}`, `decode!/1` raises.
