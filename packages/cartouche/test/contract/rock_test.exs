defmodule Cartouche.Contract.RockTest do
  @moduledoc ~S"""
  ...
  ```sh
  mix compile && mix cartouche.gen --out test/support --prefix cartouche/contract ./test/abi/Rock.json
  ```
  ...
  describe "jam" do
    test "exec_vm" do
      assert {:ok, %{f: %{beats: 22, song: "Band on the Run"}}} == Rock.exec_vm_jam(22)
    end
