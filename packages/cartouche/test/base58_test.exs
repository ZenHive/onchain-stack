defmodule Cartouche.Base58Test do
  use ExUnit.Case, async: true
  use Cartouche.Base58

  doctest Cartouche.Base58

  describe "encode/1" do
    test "empty binary" do
      assert Cartouche.Base58.encode(<<>>) == ""
    end

    test "single zero byte" do
      assert Cartouche.Base58.encode(<<0>>) == "1"
    end

    test "multiple leading zero bytes" do
      assert Cartouche.Base58.encode(<<0, 0, 0>>) == "111"
    end

    # IETF draft-msporny-base58-03 test vectors
    test "IETF: Hello World!" do
      assert Cartouche.Base58.encode("Hello World!") == "2NEpo7TZRRrLZSi2U"
    end

    test "IETF: The quick brown fox..." do
      assert Cartouche.Base58.encode("The quick brown fox jumps over the lazy dog.") ==
               "USm3fpXnKG5EUBx2ndxBDMPVciP5hGey2Jh4NDv6gmeo1LkMeiKrLJUUBk6Z"
    end

    test "IETF: leading zeros" do
      assert Cartouche.Base58.encode(Base.decode16!("0000287FB4CD", case: :upper)) == "11233QC4"
    end
    ...
    test "Bitcoin Core vectors" do
      ...
      for {hex, expected_b58} <- vectors do
        ...
        assert Cartouche.Base58.encode(binary) == expected_b58, "encode failed for hex #{hex}"
      end
    end

    # Boundary tests near 58^5
    test "boundary: zzzzy" do
      assert Cartouche.Base58.encode(Base.decode16!("271F359E", case: :upper)) == "zzzzy"
    end

    test "boundary: zzzzz (58^5 - 1)" do
      assert Cartouche.Base58.encode(Base.decode16!("271F359F", case: :upper)) == "zzzzz"
    end

    test "boundary: 211111 (58^5)" do
      assert Cartouche.Base58.encode(Base.decode16!("271F35A0", case: :upper)) == "211111"
    end

    # Solana program addresses
    test "Solana System Program (32 zero bytes)" do
      assert Cartouche.Base58.encode(<<0::256>>) == "11111111111111111111111111111111"
    end
    ...
    test "Solana Token Program" do
      ...
      assert Cartouche.Base58.encode(binary) == "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"
    end
    ...
    test "Solana Associated Token Account Program" do
      ...
      assert Cartouche.Base58.encode(binary) ==
    ...
    test "Solana Token 2022 Program" do
      ...
      assert Cartouche.Base58.encode(binary) == "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb"
    end
  end

  describe "decode/1" do
    test "empty string" do
      assert Cartouche.Base58.decode("") == {:ok, <<>>}
    end

    test "single 1" do
      assert Cartouche.Base58.decode("1") == {:ok, <<0>>}
    end

    test "multiple leading 1s" do
      assert Cartouche.Base58.decode("111") == {:ok, <<0, 0, 0>>}
    end

    # IETF draft vectors (decode direction)
    test "IETF: Hello World!" do
      assert Cartouche.Base58.decode("2NEpo7TZRRrLZSi2U") == {:ok, "Hello World!"}
    end

    test "IETF: The quick brown fox..." do
      assert Cartouche.Base58.decode("USm3fpXnKG5EUBx2ndxBDMPVciP5hGey2Jh4NDv6gmeo1LkMeiKrLJUUBk6Z") ==
               {:ok, "The quick brown fox jumps over the lazy dog."}
    end

    test "IETF: leading zeros" do
      {:ok, result} = Cartouche.Base58.decode("11233QC4")
    ...
    test "Bitcoin Core vectors" do
      ...
      for {b58, expected_hex} <- vectors do
        ...
        assert Cartouche.Base58.decode(b58) == {:ok, expected}, "decode failed for #{b58}"
      end
    end

    # Solana addresses
    test "Solana System Program" do
      assert Cartouche.Base58.decode("11111111111111111111111111111111") == {:ok, <<0::256>>}
    end
    ...
    test "Solana Token Program" do
      ...
      assert Cartouche.Base58.decode("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA") ==
               {:ok, expected}
    end

    # Error cases
    test "invalid character: 0" do
      assert Cartouche.Base58.decode("abc0def") == {:error, {:invalid_character, "0"}}
    end

    test "invalid character: O" do
      assert Cartouche.Base58.decode("O") == {:error, {:invalid_character, "O"}}
    end

    test "invalid character: I" do
      assert Cartouche.Base58.decode("I") == {:error, {:invalid_character, "I"}}
    end

    test "invalid character: l" do
      assert Cartouche.Base58.decode("l") == {:error, {:invalid_character, "l"}}
    end

    test "invalid character: +" do
      assert Cartouche.Base58.decode("+") == {:error, {:invalid_character, "+"}}
    end
  ...
  describe "decode!/1" do
    test "valid input" do
      assert Cartouche.Base58.decode!("2g") == <<0x61>>
    end

    test "raises on invalid input" do
      assert_raise ArgumentError, ~r/invalid Base58/, fn ->
        Cartouche.Base58.decode!("0invalid")
      end
    end
  end
  ...
  describe "roundtrip" do
    test "encode then decode for all Bitcoin Core vectors" do
      ...
      for hex <- hexes do
        binary = Base.decode16!(hex, case: :upper)
        assert Cartouche.Base58.decode!(Cartouche.Base58.encode(binary)) == binary
      end
    end
