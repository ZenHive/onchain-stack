...
|---|---|---|---|---|---|---|
| `build_current_thread_runtime` | release (NIF) | 10_000 | **1.2 µs** | 1.5 µs | 15 µs | 155 µs |
| `build_current_thread_runtime` | debug | 10_000 | 6.0 µs | 7.6 µs | 67 µs | — |

| `simulate_call` | calldata | median wall | min | max |
|---|---|---|---|---|
| Cheap | USDC `totalSupply()` | **3.094 s** | 2.610 s | 3.108 s |
| Expensive | Aave V3 `getUserAccountData` of `0x28C6c06298d514Db089934071355E5743bf21d60` | **4.091 s** | 3.302 s | 4.273 s |

Share of wall time, release-profile construction:

| Numerator | Cheap share | Expensive share |
|---|---|---|
| median 1.2 µs | **0.000039%** (1.2 µs / 3.094 s) | **0.000029%** (1.2 µs / 4.091 s) |
| cold 155 µs | 0.0050% | 0.0038% |
| 1 ms ceiling (the rust test's bound) | 0.032% | 0.024% |
...
A 100× faster node (no SSH tunnel, ~31 ms cheap call) would still see a
median share of 0.004%. Construction is not a visible part of wall time.
